...
const Header: React.FC<{ header: AboutContent['header'] }> = React.memo(({ header }) => (
...
const Description: React.FC<{ description: AboutContent['description'] }> = React.memo(({ description }) => (
...
const CoreValues: React.FC<{ values: AboutContent['values'] }> = React.memo(({ values }) => (
  <div className='lg:col-span-2 bg-syntara-darker rounded-lg p-6 border border-border/30'>
    ...
    <ul className='space-y-6'>
      {values.map((value, index) => (
        <li key={`${value.title}-${index}`} className='flex items-start gap-4'>
